...
for /f "tokens=1,2,3 delims=|" %%a in ("%control_node%") do (
    ...
    (
        ...
        start %%c cmd /c ssh %%a@%%b -i ../Virtual_ENV/multipass-ssh-key "cd Szakdolgozat_v2 && cd Application && chmod +x control_node_action.sh"
    )
...
for %%n in (%nodes%) do (
    for /f "tokens=1,2,3 delims=@" %%a in ("%%n") do (
        ...
        (
            ...
            start %%c cmd /c ssh %%a@%%b -i ../Virtual_ENV/multipass-ssh-key "cd Szakdolgozat_v2 && cd Application && chmod +x node_action.sh"
        )
