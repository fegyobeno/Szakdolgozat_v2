...
for /f "tokens=1,2,3 delims=|" %%a in ("%control_node%") do (
    ...
    (
        ...
        ssh %%a@%%b -i ../Virtual_ENV/multipass-ssh-key "cd Szakdolgozat_v2 && cd Application && chmod +x control_node_action.sh && ./control_node_action.sh"
    )
...
for %%n in (%nodes%) do (
    for /f "tokens=1,2,3 delims=@" %%a in ("%%n") do (
        ...
        (
            ...
            ssh %%a@%%b -i ../Virtual_ENV/multipass-ssh-key "cd Szakdolgozat_v2 && cd Application && chmod +x node_action.sh && ./node_action.sh"
        )
